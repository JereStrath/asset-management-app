...
import 'maintenance_schedule_screen.dart'; // Import MaintenanceScheduleScreen
...
class _AddAssetScreenState extends State<AddAssetScreen> {
  ...
  Future<void> _fetchCategories() async {
    try {
      ...
      if (snapshot.docs.isNotEmpty) {
        setState(() {
          _selectedCategory = snapshot.docs.first.id;
        });
      }
    } catch (e) {
  ...
  Future<void> _fetchDepartments() async {
    try {
      ...
      if (snapshot.docs.isNotEmpty) {
        setState(() {
          _selectedDepartment = snapshot.docs.first.id;
        });
      }
    } catch (e) {
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      try {
        ...
        // 1. Add asset to Firestore *and wait for it to complete*.
        final docRef = await _firestore.collection('assets').add(asset);
        final assetId = docRef.id; // Get the Firestore-generated ID

        // 2. Create initial history entry
        final history = AssetHistory(
          id: _uuid.v4(),
          assetId: assetId, // Use the Firestore-generated ID
          action: 'CREATED',
          userId: user.uid,
        ...
        // 3. Add history entry *and wait*.
        await _firestore
            .collection('assets')
            .doc(assetId) // Use the Firestore-generated ID
            .collection('history')
            .add(history.toMap());
        ...
        // 4. Show QR code screen (optional, but wait if you do)
        ...
        // 5. *Now* navigate to MaintenanceScheduleScreen, *after* everything else.
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => MaintenanceScheduleScreen(
              assetId: assetId, // Use the Firestore-generated ID
              assetName: _assetNameController.text,
            ),
          ),
