...
class _AssetManagementScreenState extends State<AssetManagementScreen> {
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => ShareAssetsScreen()),
              );
            },
          ),
        ],
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  onTap: () {
                    print("AssetManagementScreen - onTap - asset.id: ${asset.id}");
                    Navigator.push(
                      context,
                      MaterialPageRoute(
  ...
  void _showQRCode(BuildContext context, String barcode) {
    print('Showing QR Code for: $barcode'); // Debug print
  ...
  Future<void> updateAsset(Asset asset) async {
    // Option 1: Update lastLogin (e.g., when the asset is viewed)
    asset.lastLogin = Timestamp.now();
    await _firestore.collection('assets').doc(asset.id).update(asset.toMap());

    // Option 2: Preserve existing lastLogin (don't include it in the update)
    // await _firestore.collection('assets').doc(asset.id).update({
    //   'name': asset.name,
    //   'description': asset.description,
    //   // ... other fields you want to update ...
    //   // DO NOT include lastLogin here
    // });
  }
}